...
public class ItemsDistribution {
    ...
    public ItemsDistribution(Collection<DiagramObject> items, AnalysisType analysisType) {
        this.map = new HashMap<String, MapSet<RenderType, DiagramObject>>();
        ...
        if (item != null) {
            ...
            if (item.getIsFadeOut() != null) {
                ...
                if (analysisType.equals(AnalysisType.NONE)) {
                    ...
                    DatabaseObject dbObject = item.getDatabaseObject();
                    boolean isHit = false;
                    if (dbObject != null) {
                        if (dbObject instanceof PhysicalEntity) {
                            isHit = ((PhysicalEntity) dbObject).isHit();
                        }else if (dbObject instanceof Pathway) {
                            isHit = ((Pathway) dbObject).isHit();
                        }
                        if (isHit) {
                            switch (analysisType) {
    ...
    private MapSet<RenderType, DiagramObject> getOrCreate(String renderableClass) {
        ...
        if (map == null) {
            map = new MapSet<RenderType, DiagramObject>();
